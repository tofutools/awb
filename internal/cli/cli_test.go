...
// --exclude-epic is a CLI convenience shared by every issue listing. It is
// translated to the existing inclusive type filter, so direct and remote mode
// still use the same backend operation and the HTTP API gains no new option.
func TestListingsExcludeEpics(t *testing.T) {
	h := newHarness(t)
	...
	assert.Contains(t, stderr, "--type and --exclude-epic are mutually exclusive")
}
