...
// CompactLine renders an issue as the one-line form of SPEC §4.1, designed to
...
// their position, and appear in this fixed order when present: @<assignee>, one
// #<label> per label in sorted order, !blocked and, when withBlockers is set,
// one blocked-by:<id> per entry of Blockers, in sorted order. That last group
// is for awb blocked, which is the listing whose point is the blockers.
//
...
// CompactProjectLine renders a project as "<key> <active_issues> <name>", where
// the name is a JSON string (SPEC §4.1).
...
// therefore unindented. That prefix is the one thing that may precede the id, so
// a consumer strips the leading spaces, counts them to recover the depth, and
...
// read; SetEscapeHTML(false) turns that off. Encode appends a newline, which is
// trimmed.
func jsonString(s string) string {
	...
	if err := enc.Encode(s); err != nil {
		// A Go string always encodes; invalid UTF-8 cannot reach here, having
		// been refused by the text gate of SPEC §2.6.
