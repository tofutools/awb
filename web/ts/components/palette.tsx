  ...
  return (
    <>
      <Button
        ...
        title={paletteTrigger.title}
        aria-keyshortcuts={paletteTrigger.keyShortcuts}
        onClick={() => setOpen(true)}
      >
        {paletteTrigger.label}
        <kbd>{paletteShortcutHint()}</kbd>
      </Button>
      {open && <PaletteDialog route={route} onClose={() => setOpen(false)} />}
    </>
  );
}
function PaletteDialog({
  ...
  const [remote, setRemote] = useState<PaletteCommand[]>([]);
  const [status, setStatus] = useState<SuggestionState>("idle");
  const [selected, setSelected] = useState(0);
  const input = useRef<HTMLInputElement>(null);
  const list = useRef<HTMLDivElement>(null);
  const search = useRef<SuggestionSearch<PaletteCommand>>();
  const go = (href: string) => {
    onClose();
    location.hash = href;
  };
  const all: PaletteCommand[] = namedDestinations.map((d) => ({
    id: `view:${d.id}`,
    label: `Go to ${d.label}`,
    hint: "View",
    keywords: d.keywords,
  ...
  useEffect(() => {
    const next = new SuggestionSearch(
      async (q, s) => navigationResultCommands(await api.navigation(q, s), go),
      (status, rows) => {
        setStatus(status);
        setRemote(rows);
        setSelected(0);
  ...
  useEffect(() => {
    list.current
      ?.querySelector<HTMLElement>(`#palette-option-${selected}`)
  ...
  return (
    <Modal title="Commands" className="command-palette" onClose={onClose}>
        ...
        <input
          ...
          onKeyDown={(e) => {
            ...
            if (
              ...
              isPlainPaletteBoundaryKey(e)
            ) {
              e.preventDefault();
              setSelected(
                nextPaletteSelection(
                  e.key as PaletteNavigationKey,
                  selected,
                  commands.length,
                  list.current
                    ? visiblePalettePageSize(
      ...
      <div class="command-palette-status" role="status">
        {status === "loading"
          ? "Searching…"
          : status === "error"
            ? "Search unavailable"
        ...
        {commands.map((command, index) => (
          <div key={command.id}>
            {(index === 0 || commands[index - 1].group !== command.group) && (
              <div class="command-palette-group">{command.group}</div>
            )}
            <div
            ...
            >
              ...
              <span class="command-palette-hint">{command.hint}</span>
            </div>
          </div>
        ))}
      </div>
