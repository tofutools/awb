...
func TestRemoteProjectLifecycleParity(t *testing.T) {
	requests := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch requests {
		case 1:
			assert.Equal(t, "/api/projects/awb/archive", r.URL.Path)
		...
		case 2:
			...
			_, _ = w.Write([]byte(`[]`))
		case 3:
			assert.Equal(t, "/api/projects/awb/activity", r.URL.Path)
	...
	project, err := client.ArchiveProject(t.Context(), "awb", `"v1"`)
	require.NoError(t, err)
	assert.Equal(t, domain.ProjectArchived, project.State)
	_, err = client.ListProjectsByState(t.Context(), "", domain.ProjectsArchived, domain.DefaultProjectSort, nil, nil)
	require.NoError(t, err)
	_, err = client.ListProjectActivity(t.Context(), "awb", nil, nil)
	require.NoError(t, err)
