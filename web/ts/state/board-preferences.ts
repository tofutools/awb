...
export function saveCollapsedBoardLanes(ref: string, workspaces: Set<string>): void {
...
export function hiddenBoardEpicEntries(identity: string, ref: string): HiddenBoardEpic[] {
  const validID = (value: string): boolean => /^[a-z][a-z0-9-]*-[0-9a-f]{6}$/.test(value);
  const entries = new Map<string, HiddenBoardEpic>();
  try {
    const stored: unknown = JSON.parse(localStorage.getItem(boardHiddenEpicsKey(identity, ref)) ?? "[]");
...
export function hiddenBoardEpics(identity: string, ref: string): Set<string> {
  return new Set(hiddenBoardEpicEntries(identity, ref).map((entry) => entry.id));
}

export function saveHiddenBoardEpics(identity: string, ref: string, epics: Set<string>, added?: HiddenBoardEpic): void {
  const entries = new Map(hiddenBoardEpicEntries(identity, ref).map((entry) => [entry.id, entry]));
  ...
  try { localStorage.setItem(boardHiddenEpicsKey(identity, ref), JSON.stringify(stored)); } catch { /* presentation state is best-effort */ }
}
...
export function defaultBoardPreferences(identity: string): DefaultBoardPreferences {
  ...
  try {
    const parsed: unknown = JSON.parse(localStorage.getItem(defaultBoardPreferencesKey(identity)) ?? "null");
...
export function saveDefaultBoardPreferences(identity: string, value: DefaultBoardPreferences): void {
  try { localStorage.setItem(defaultBoardPreferencesKey(identity), JSON.stringify(value)); } catch { /* preference is best-effort */ }
}
...
export function defaultBoardView(identity: string): BoardView {
  return { id: "default", name: "Default board", owner: identity, shared: false,
    ...defaultBoardPreferences(identity), created_at: "", updated_at: "" };
}

export function effectiveDefaultBoardView(identity: string, route: Route): BoardView {
  const view = defaultBoardView(identity); const workspaces = route.query.getAll("workspace");
