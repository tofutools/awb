...
function useWorkspaceManager(
  identity: string,
): ReturnType<typeof useResource<boolean>> {
...
function WorkspaceSortButton({
  route,
  column,
}: {
  route: Route;
  column: (typeof workspaceColumns)[number];
}) {
  ...
  return (
    ...
    >
      {column.label}
      <span
        class={`sort-arrow${active ? "" : " sort-hint"}`}
        aria-hidden="true"
      >
...
function MobileWorkspaceSort({ route }: { route: Route }) {
  ...
  const value = values.has(route.query.get("sort") ?? "")
...
function WorkspaceTable({
  ...
  const state = sortState(route.query.get("sort"), workspaceSortKeys, "key");
  return (
    <table class="listing-table workspace-table">
      <thead>
        <tr>
        ...
        {workspaces.map((workspace) => (
          <tr key={workspace.key}>
            <td data-label="Workspace">
              <NameLink
                href={`#/workspaces/${encodeURIComponent(workspace.key)}`}
                id={workspace.key}
                title={workspace.name}
              />
              {workspace.state === "archived" && (
                <span class="listing-badge archived-badge">Archived</span>
              )}
            ...
            <td data-label="Open">
              <span class="open-count">{workspace.active_issues}</span>
            </td>
            <td data-label="Updated">
              <UpdatedTime timestamp={workspace.updated_at} />
            </td>
          </tr>
        ))}
      </tbody>
...
function WorkspaceCreateForm({ hidden }: { hidden: boolean }) {
  ...
  return (
    <form
      ref={formRef}
      class="edit-panel workspace-create-panel"
      ...
      onSubmit={async (event) => {
        ...
        if (ok && created !== undefined)
          location.hash = `#/workspaces/${encodeURIComponent(created.key)}`;
      }}
    >
      <h2>Create workspace</h2>
      <p class="muted">
        The key becomes every issue ID prefix in this workspace and cannot be
        changed. Issues cannot move between workspaces.
      </p>
      <Field label="Key">
        <input
          name="key"
      ...
      <p class="workspace-key-preview muted">
        {key === ""
          ? "Issue IDs will use this key as their prefix."
          : `Issue IDs will start with ${key}-.`}
      </p>
...
async function loadWorkspacePage(route: Route): Promise<Page<Workspace>> {
  ...
  const requested = pageNumber(route.query);
  const lifecycle =
    route.query.get("state") === "archived" ? "archived" : "active";
  ...
  if (
    sort !== null &&
    workspaceSortKeys.flatMap((key) => [key, `-${key}`]).includes(sort)
  ) {
...
export function WorkspacesPage({ route }: PageProps) {
  ...
  if (manager.error !== undefined)
    return <ErrorMessage error={manager.error} />;
  if (resource.data === undefined || manager.data === undefined)
    return <Loading />;
  const page = resource.data;
  const tabHref = (state: "active" | "archived") => {
    const query = new URLSearchParams(route.query);
    query.delete("page");
    if (state === "active") query.delete("state");
    else query.set("state", state);
  ...
  return (
    <div>
      <div class="workspaces-heading">
        ...
        {manager.data && (
          <Button
            ...
            aria-controls="workspace-creator"
      ...
      <div class="workspace-state-tabs">
        <a
          href={tabHref("active")}
          class={lifecycle === "active" ? "active" : undefined}
          aria-current={lifecycle === "active" ? "page" : undefined}
        >
          Active
        </a>
        <a
          href={tabHref("archived")}
          class={lifecycle === "archived" ? "active" : undefined}
          aria-current={lifecycle === "archived" ? "page" : undefined}
        >
          Archived
        </a>
      </div>
      <div class="listing">
        <div class="listing-tools">
          <SearchInput
            value={filter}
            placeholder="Filter all workspaces…"
            onInput={setFilter}
          />
          <span class="filter-count">
            {page.total} workspace{page.total === 1 ? "" : "s"}
          </span>
          <div class="listing-actions">
            <MobileWorkspaceSort route={route} />
            <span class="mobile-updated-display">
              Updated
              <UpdatedDisplayControl />
            </span>
            <Pagination route={route} total={page.total} />
          </div>
        </div>
        <div class="listing-host">
          {page.rows.length === 0 ? (
            <p class="empty">
              {route.query.get("filter") !== null
                ? "No workspaces match this filter."
                : lifecycle === "archived"
                  ? "No archived workspaces."
                  : "No workspaces yet. Create one above or with: awb workspace create <key>"}
            </p>
          ) : (
            <WorkspaceTable route={route} workspaces={page.rows} />
          )}
        </div>
      </div>
    </div>
...
function UserTable({
  ...
  return (
    <table class="listing-table user-table">
      <thead>
        <tr>
          {["User", "Memberships", "Activity", "Roles"].map((label) => (
      ...
      <tbody>
        {users.map((user) => (
          <tr key={user.name}>
            <td data-label="User">
              {manageable ? (
                <a class="user-name" href={userEditorHref(user.name)}>
              ...
              <div class="user-workspaces">
                {user.workspaces.length === 0 ? (
                  ...
                  user.workspaces.map((membership) => (
                    <span
                      class="listing-badge user-workspace"
                      title={`${membership.access} access`}
                    >
            ...
            <td data-label="Activity">
              <div class="user-workspaces">
                {user.activity_workspaces.length === 0 ? (
                  <span class="muted">—</span>
                ) : (
                  user.activity_workspaces.map((workspace) => (
                    <span class="listing-badge user-workspace">
                      {workspace}
                    </span>
                  ))
                )}
            ...
            <td data-label="Roles">
              <div class="user-roles">
                {user.workspace_admin && (
                  <span class="listing-badge">workspace admin</span>
                )}
                {user.user_admin && (
                  <span class="listing-badge">user admin</span>
                )}
                {!user.workspace_admin && !user.user_admin && (
...
function UserIdentity({ user }: { user: DirectoryUser }) {
  return (
    <span class="user-identity">
      <span class="user-full-name">{user.full_name || user.name}</span>
      {user.full_name !== "" && <span class="muted">@{user.name}</span>}
    </span>
  );
}
...
async function loadUsers(route: Route): Promise<Page<DirectoryUser>> {
  ...
  const filters: UserFilters = {
    limit: size,
    offset: (pageNumber(route.query) - 1) * size,
  };
  const filter = route.query.get("filter");
  if (filter !== null && filter !== "") filters.filter = filter;
  ...
  return (
    <div>
      <div class="directory-heading">
        <h1>Users</h1>
        {app.mayManageUsers && (
          <a href={userCreateHref} class="primary-button">
            Add user
          </a>
        )}
      </div>
      <div class="listing">
        <div class="listing-tools">
          ...
          />
          <span class="filter-count">
            {resource.data.total} user{resource.data.total === 1 ? "" : "s"}
          </span>
        ...
        <div class="listing-host">
          {resource.data.rows.length === 0 ? (
            ...
            <UserTable
              users={resource.data.rows}
              manageable={app.mayManageUsers}
            />
          )}
        </div>
      </div>
...
function FormStatus({
  ...
  return (
    <p
      class={`profile-form-message${error === undefined ? "" : " form-error"}`}
      aria-live="polite"
...
function UserAccountForm({
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          if (updated.name === app.identity && !updated.user_admin)
            location.hash = "#/users";
          else await reload();
        } catch (caught) {
          if (caught instanceof ApiError && caught.status === 412) {
            app.notify(
              "This account changed elsewhere. The latest values have been loaded; review them and try again.",
              true,
            );
            await reload();
    ...
    >
      ...
      <label class="check-field user-role-field">
        <input
          type="checkbox"
          checked={workspaceAdmin}
          onChange={(event) => setWorkspaceAdmin(event.currentTarget.checked)}
        />
      ...
      <label class="check-field user-role-field">
        <input
          type="checkbox"
          checked={userAdmin}
          onChange={(event) => setUserAdmin(event.currentTarget.checked)}
        />
...
function UserPasswordResetForm({
  ...
  return (
    ...
    >
      <p class="profile-form-help">
        Set a new password. The current password is never shown.
      </p>
...
function UserMembershipList({ user }: { user: User }) {
  return (
    <ul class="profile-workspaces">
      {user.workspaces.length === 0 ? (
        <li class="empty">No workspace memberships.</li>
      ) : (
        user.workspaces.map((membership) => (
          <li class="profile-workspace" key={membership.workspace}>
            <a
              href={`#/workspaces/${encodeURIComponent(membership.workspace)}/members`}
              class="profile-workspace-name"
            >
              {membership.workspace}
            </a>
  ...
  return (
    ...
    >
      ...
      <Button
        type="submit"
        class="danger-button"
        disabled={busy || confirmation !== user.name}
      >
        Delete user
      </Button>
...
export function UserPage({ route }: PageProps) {
  ...
  const resource = useResource(async () => {
    await app.refreshCaller();
    if (!app.mayManageUsers)
      throw new ApiError(
        403,
        "Only a user administrator may administer accounts.",
      );
  ...
  return (
    <div class="profile-view user-admin-view">
      <a href="#/users" class="detail-back-link">
        ← Users
      </a>
      <div class="profile-heading user-admin-heading">
        ...
        <div>
          ...
          <p class="lede">
            {user.full_name === ""
              ...
              : `@${user.name} · Account administration`}
          </p>
        </div>
      </div>
      <div class="user-admin-grid">
        ...
        <section class="profile-card">
          ...
          <UserPasswordResetForm user={user} reload={resource.reload} />
        </section>
        <section class="profile-card">
          <h2>Workspaces</h2>
          <p class="profile-form-help">
            Workspace memberships are read-only here. Manage access on each
            workspace's Members page.
          </p>
          <UserMembershipList user={user} />
        </section>
        <section class="profile-card">
        ...
        <section class="profile-card user-delete-card">
  ...
  return (
    <div class="profile-view user-admin-view">
      ...
      <div class="settings-heading">
        <h1>Add user</h1>
        <p class="lede">Create an account and its initial roles.</p>
      </div>
      <section class="profile-card user-create-card">
        <h2>Account</h2>
        <form
          class="user-admin-form user-create-form"
          aria-busy={mutation.busy || undefined}
          onSubmit={async (event) => {
            event.preventDefault();
            setMismatch(false);
            ...
            const ok = await mutation.run(async () => {
              created = await api.createUser(body);
            });
            if (ok && created !== undefined) {
        ...
        >
          ...
          <Field label="Full name">
            <input name="full_name" maxLength={500} autoComplete="name" />
          </Field>
          ...
          <label class="check-field user-role-field">
            <input name="workspace_admin" type="checkbox" />
            <span>Workspace administrator</span>
          </label>
          <label class="check-field user-role-field">
  ...
  return useResource<WorkspaceBundle>(async () => {
    const loadCurrentUser = async (): Promise<User | null> => {
      ...
      try {
        ...
        // A no-auth server attributes activity to an identity without
        // requiring a corresponding account row.
    ...
    const [workspace, activity, members, currentUser] = await Promise.all([
      ...
      api.workspaceMembers(key),
      loadCurrentUser(),
    ]);
    const canManage =
      identity === "" || currentUser === null || currentUser.workspace_admin;
    return {
      workspace,
...
function WorkspaceEditForm({
  ...
  return (
    <form
      ref={formRef}
      class="edit-panel workspace-edit-form"
      hidden={hidden}
      aria-busy={mutation.busy || undefined}
      onSubmit={async (event) => {
        event.preventDefault();
...
function WorkspaceLifecycleCard({
  ...
  return (
    <section
      class="workspace-lifecycle-card"
      aria-busy={mutation.busy || undefined}
    >
      <h2>Lifecycle</h2>
      {workspace.state === "archived" ? (
        <>
          <p>
            Issues, comments, attachments, transitions and relations are
            read-only while this workspace is archived. Issues remain in this
            workspace and cannot be transferred elsewhere.
          </p>
          {workspace.archived_at !== "" && (
            <p class="muted">
              Archived
              {workspace.archived_by === ""
                ? ""
                : ` by @${workspace.archived_by}`}{" "}
              · <UpdatedTime timestamp={workspace.archived_at} />
            </p>
          )}
          {canManage && (
            <Button
              ...
              onClick={() =>
                void mutate(() => api.restoreWorkspace(workspace.key))
              }
            >
              Restore workspace
            </Button>
        ...
        <>
          <p>
            Archive this workspace to remove it from everyday discovery and make
            its retained work read-only. Its issues keep their stable
            workspace-prefixed IDs.
          </p>
          {canManage && (
            <form
              class="workspace-archive-form"
              onSubmit={(event) => {
                event.preventDefault();
                if (confirmation === workspace.key)
                  void mutate(() => api.archiveWorkspace(workspace.key));
              }}
            >
              <label>Type {workspace.key} to confirm</label>
              <input
                placeholder={workspace.key}
                aria-label={`Type ${workspace.key} to confirm`}
                value={confirmation}
                onInput={(event) => setConfirmation(event.currentTarget.value)}
              />
              <Button
                type="submit"
                class="danger-button archive-button"
                disabled={mutation.busy || confirmation !== workspace.key}
              >
                Archive workspace
              </Button>
            </form>
      ...
      {activity.length > 0 && (
        <>
          ...
          <ol class="workspace-lifecycle-history">
            {activity.map((entry) => (
              <li>
                {entry.action === "archived" ? "Archived" : "Restored"}
                {entry.actor === "" ? "" : ` by @${entry.actor}`} ·{" "}
                <UpdatedTime timestamp={entry.created_at} />
              </li>
            ))}
          </ol>
        </>
...
interface MembershipAccessControlProps {
  member: Membership;
  members: Membership[];
  identity: string;
  busy: boolean;
...
function MembershipAccessControl({
  member,
  members,
  identity,
  busy,
  change,
}: MembershipAccessControlProps) {
  const [access, setAccess] = useState(member.access);
  useEffect(() => setAccess(member.access), [member.access]);

  return (
    <select
      aria-label={`Access for @${member.user}`}
      value={access}
      disabled={busy}
      onChange={async (event) => {
        const next = event.currentTarget.value as Membership["access"];
        setAccess(next);
        if (next === member.access) return;
        if (
          !(await confirmMutation(
            "Change workspace access?",
            membershipChangeConfirmation(member, members, identity, next),
          ))
        ) {
        ...
        const changed = await change(
          () => api.setWorkspaceMember(member.workspace, member.user, next),
          `@${member.user} now has ${next} access to workspace ${member.workspace}.`,
        );
        if (!changed) setAccess(member.access);
      }}
    >
...
function WorkspaceMembershipSection({
  workspace,
  members,
  currentUser,
  reload,
  ...
  const manageable =
    unrestricted ||
    mayManageWorkspaceMembership(identity, currentUser, workspace.key, members);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<unknown>();
  ...
  const change = async (
    ...
    try {
      await operation();
      notify(success);
      if (options.redirect) location.hash = "#/workspaces";
      else await reload();
      return true;
    } catch (caught) {
      if (
        options.refreshNotFound &&
        caught instanceof ApiError &&
        caught.status === 404
      ) {
        notify(
          "Membership changed elsewhere. The current member list has been reloaded.",
          true,
        );
        await reload();
      } else if (
        ...
        notify(
          "That user was added elsewhere. The current member list has been reloaded.",
          true,
        );
        await reload();
      } else setError(caught);
  ...
  return (
    <section class="profile-card membership-card" aria-busy={busy || undefined}>
      <div class="membership-heading">
        <div>
          <h2>Workspace members</h2>
          <p class="membership-help">
            Membership grants access to this workspace. It is separate from each
            user's ignored-workspace preference.
          </p>
        </div>
        <span class="membership-count">{members.length}</span>
      </div>
      {manageable ? (
        <form
          class="compact-editor membership-editor"
          onSubmit={(event) => {
            event.preventDefault();
            ...
            void change(
              () => api.addWorkspaceMember(workspace.key, user, newAccess),
              `@${user} was added with ${newAccess} access to workspace ${workspace.key}.`,
        ...
        >
          <Autocomplete
            ...
            load={async (query, signal) =>
              membershipSuggestions(
                (await api.users({ filter: query, limit: 8 }, signal)).rows,
                members,
              )
            }
          />
          <select
            ...
            value={newAccess}
            onChange={(event) =>
              setNewAccess(event.currentTarget.value as Membership["access"])
            }
          >
            <option value="regular">Regular access</option>
            <option value="admin">Administrator</option>
        ...
        <p class="membership-help">
          Workspace administrators can change membership and access.
        </p>
      )}
      {error !== undefined && <ErrorMessage error={error} />}
      {members.length === 0 ? (
        <p class="empty">
          No stored members. Global workspace administrators still have access.
        </p>
      ) : (
        <table class="listing-table membership-table">
          <thead>
            <tr>
              ...
              <tr key={member.user}>
                <td data-label="User">
                  <span class="user-name">
                    ...
                    <span class="user-identity">@{member.user}</span>
                  </span>
                </td>
                <td data-label="Access">
                  {manageable ? (
                    ...
                    <span class="listing-badge">{member.access}</span>
                  )}
                </td>
                <td data-label="Actions">
                  {manageable ? (
                    <Button
                      class="danger-button membership-remove"
                      aria-label={`Remove @${member.user} from workspace ${workspace.key}`}
                      disabled={busy}
                      onClick={async () => {
                        if (
                          !(await confirmMutation(
                        ...
                        const losesAccess =
                          ...
                          currentUser?.workspace_admin !== true;
                        await change(
                          () =>
                          ...
                          { refreshNotFound: true, redirect: losesAccess },
                        );
                      }}
                    >
                      Remove
    ...
    <div class="workspace-view">
      {workspace.state === "archived" && (
        <div class="workspace-archive-banner" role="status">
          ...
          history. Restore it to resume work inside the same workspace boundary.
        </div>
      )}
      <div class="detail-heading">
        <div>
          <div class="issue-key">{workspace.key}</div>
      ...
      <a
        class="action"
        href={`#/issues?workspace=${encodeURIComponent(workspace.key)}${workspace.state === "archived" ? "&include-archived=true&include-closed=true" : ""}`}
      >
        {workspace.state === "archived"
          ? "View this workspace's historical issues"
          : "View this workspace's issues"}
      </a>
      ...
      <WorkspaceMembershipSection
        ...
        reload={resource.reload}
        unrestricted={currentUser === null && canManage}
      />
    </div>
  );
}
...
export function WorkspaceMembersPage({ route }: PageProps) {
  ...
  const resource = useResource(async () => {
    ...
    const preference = preferences.find(
      (candidate) => candidate.workspace.key === key,
    );
    if (preference === undefined)
      throw new ApiError(404, `no such workspace: ${key}`);
  ...
  return (
    <div class="workspace-view membership-admin-view">
      <div class="detail-heading">
        <div>
          <div class="issue-key">{preference.workspace.key}</div>
          <h1>{preference.workspace.name}</h1>
          <p class="lede">
            {preference.ignored
              ? "Ignored workspace administration"
              : "Workspace administration"}
          </p>
        </div>
        <a href="#/settings" class="secondary-button">
          Back to settings
        </a>
      </div>
      <WorkspaceMembershipSection
  ...
  return (
    <ul class="profile-workspaces">
      {workspaces.map((workspace) => {
        ...
        const access = user.workspace_admin
          ? "admin"
          : memberships.get(workspace.key);
        return (
          <li class="profile-workspace" key={workspace.key}>
            ...
            {workspace.name !== "" && (
              <span class="profile-workspace-title">{workspace.name}</span>
            )}
            {access !== undefined && (
              <span class="listing-badge">{access}</span>
            )}
          </li>
        );
      })}
...
function FullNameForm({
  ...
  return (
    <form class="profile-name-form" onSubmit={submit}>
      ...
      />
      <p class="profile-form-help">
        Optional. Shown with @{user.name} in the user directory.
      </p>
      <Button class="profile-submit" type="submit" disabled={mutation.busy}>
        Save full name
      </Button>
      <p
        class={`profile-form-message${mutation.error === undefined ? "" : " form-error"}`}
        aria-live="polite"
      >
...
function PasswordForm({ user }: { user: User }): JSX.Element {
  ...
  const submit = async (
    ...
    const completed = await mutation.run(() =>
      api.updateUser(user.name, { password }),
    );
    if (completed) {
      form.reset();
  ...
  return (
    <form class="profile-password-form" onSubmit={submit}>
      ...
      <p
        class={`profile-form-message${error === "" ? "" : " form-error"}`}
        aria-live="polite"
      >
        {error || message}
      </p>
    ...
    <div class="profile-view">
      ...
      <section class="profile-card">
        ...
        <div class="profile-roles">
          {accountRoles(user).map((role) => (
            <span class="listing-badge" key={role}>
              {role}
            </span>
          ))}
        </div>
...
export function ProfilePage({ route }: PageProps): JSX.Element {
  ...
  return (
    <ProfileContent
      initialUser={resource.data.user}
...
function WorkspacePreferenceRow({
  ...
  const toggle = async (): Promise<void> => {
    ...
    const completed = await mutation.run(async () => {
      updated = await api.setWorkspaceIgnored(
        preference.workspace.key,
        !preference.ignored,
      );
    });
  ...
  return (
    <li
      class={`workspace-preference-row${preference.ignored ? " ignored" : ""}`}
    >
      ...
      <span
        class={`workspace-preference-state ${preference.ignored ? "ignored-state" : "active-state"}`}
      >
...
function IgnoredWorkspacesSettingsCard({
  initialWorkspaces,
}: {
  initialWorkspaces: WorkspacePreference[];
}): JSX.Element {
  ...
  const update = (updated: WorkspacePreference): void => {
    setWorkspaces((current) =>
      current.map((preference) =>
        preference.workspace.key === updated.workspace.key
          ? updated
          : preference,
      ),
    );
    ...
    <section class="profile-card ignored-workspaces-card">
      <div class="ignored-workspaces-heading">
        <div>
          ...
          <p>
            Ignored workspaces are hidden from listings, search, counts, and
            navigation. They always remain available here so you can re-enable
            them.
          </p>
        </div>
        <span class="ignored-summary">
          {workspacePreferenceSummary(workspaces)}
        </span>
      </div>
      ...
      {visible.size === 0 && (
        <p class="workspace-preference-empty empty">
          No authorized workspaces match your search.
        </p>
      )}
    </section>
  );
...
export function SettingsPage({ route }: PageProps): JSX.Element {
  ...
  const [paginationAutoHide, setPaginationAutoHide] = useState(() =>
    readPaginationAutoHide(storage),
  );
  ...
  return (
    <div class="profile-view settings-view">
      ...
      <section class="profile-card">
        ...
        <label class="settings-preference">
          ...
          <span class="settings-preference-copy">
            <span class="settings-preference-title">
      ...
      {resource.data !== null && (
        <IgnoredWorkspacesSettingsCard initialWorkspaces={resource.data} />
      )}
    </div>
  );
}
