...
export function IssueFields({
  ...
  issue?: Pick<
    Issue,
    "title" | "description" | "commit_hash" | "pull_request_url"
  >;
}) {
    ...
    <>
      ...
      <div class="edit-field-row">
        <Field label="Commit">
          <input
            name="commit_hash"
            defaultValue={issue?.commit_hash ?? ""}
        ...
        <Field label="Pull request">
          <input
            ...
            defaultValue={issue?.pull_request_url ?? ""}
            type="url"
            maxLength={1000}
            placeholder="https://…"
          />
...
export function issueFields(form: HTMLFormElement) {
  ...
  return {
    ...
    commit_hash: String(values.get("commit_hash") ?? ""),
    pull_request_url: String(values.get("pull_request_url") ?? ""),
  };
}
export function FilePicker({ onFiles }: { onFiles: (files: File[]) => void }) {
  ...
  return (
    <div
      class={`compact-editor attachment-editor ${drag ? "drag-active" : ""}`}
      onDragOver={(e) => {
        e.preventDefault();
        setDrag(true);
      }}
    ...
    >
      <label class="attachment-picker">
        Drop files here or <span class="attachment-browse">browse</span>
        <input
          type="file"
          multiple
...
export function RelationFields({
  ...
  onAdd: (type: Relation["type"], other: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const [type, setType] = useState<Relation["type"]>("blocked-by");
  ...
  return open ? (
    <div class="relation-fields">
      <select
        ...
        value={type}
        onChange={(e) => setType(e.currentTarget.value as Relation["type"])}
      >
        {["blocked-by", "has-parent", "discovered-from", "related"].map(
          (type) => (
      ...
      <Autocomplete
        value={other}
        onValue={setOther}
        aria-label="Other issue ID"
        placeholder="Other issue ID"
        load={async (q, s) =>
          (await api.issueSuggestions(q, s)).rows.map((i) => ({
            value: i.id,
            label: i.id,
            detail: i.title,
          }))
    ...
    <div class="relation-disclosure-row">
      <Button class="relation-disclosure" onClick={() => setOpen(true)}>
        + Add relation
      </Button>
      <span class="relation-hint">Create a dependency or association</span>
    </div>
  );
}
export function IssueCreateButton({
  label = "New issue",
  workspace,
  epic,
  assignToMe = false,
...
function CreateIssue({
  ...
  const stage = () => {
    const result = stagedLabel(label, labels);
    if (result.error !== undefined) {
      setError(result.error);
      return null;
    }
    const next = [...labels, result.label];
    setLabels(next);
    setLabel("");
    setError(undefined);
    return next;
  ...
  return (
    <Modal title="New issue" className="issue-create-dialog" onClose={onClose}>
      <ErrorMessage error={resource.error} />
      {!resource.data ? (
        <Loading />
      ) : resource.data.rows.length === 0 ? (
        <p>Create a workspace before creating an issue.</p>
      ) : (
        <form
          ...
          onSubmit={(e) => {
            ...
            void mutation.run(async () => {
              ...
              const failures = uploads.flatMap((result, index) =>
                result.status === "rejected"
                  ? [
                      `${files[index].name} (${result.reason instanceof Error ? result.reason.message : String(result.reason)})`,
                    ]
                  : [],
              );
              notify(
        ...
        >
          ...
          <div class="edit-field-row">
            ...
            <Field label="Type">
              ...
              >
                {["task", "feature", "bug", "epic", "chore"].map((t) => (
          ...
          <div class="issue-create-resources">
            <section class="issue-create-resource issue-create-labels">
              <h3>Labels</h3>
              <div class="issue-create-label-list">
                {labels.map((item) => (
                  <span class="editable-chip" key={item}>
                    <span class="listing-badge label">{item}</span>
                    <Button
                      class="chip-remove"
                      aria-label={`Remove label ${item}`}
                      onClick={() =>
                        setLabels(labels.filter((v) => v !== item))
                      }
                    >
                      ×
                    </Button>
              ...
              <div class="compact-editor issue-create-label-editor">
                <Autocomplete
                  value={label}
                  onValue={setLabel}
                  aria-label="Label"
            ...
            <section class="issue-create-resource">
              <h3>Relations</h3>
              <ul class="relations resource-list issue-create-resource-list">
                ...
                onAdd={(type, other) => {
                  ...
                  if (
                    relations.some(
                      (r) =>
                        r.type === type &&
                        (r.other === other || type === "has-parent"),
                    )
                  ) {
                    setError("That relation or parent is already staged.");
                    return;
                  }
                  setRelations([...relations, { type, other }]);
                }}
              />
            </section>
            <section class="issue-create-resource">
              <h3>Attachments</h3>
              <ul class="attachments resource-list issue-create-resource-list">
                {files.map((file) => (
                  <li key={file.name}>
                    {file.name}
                    <Button
