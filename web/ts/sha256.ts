// sha256 does not rely on WebCrypto, which browsers expose only in secure
// contexts. AWB deliberately supports explicitly opted-in cleartext serving.
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const t1 = (q + s1 + ((e & f) ^ (~e & g)) + k[i] + w[i]) | 0;
      const s0 = rotate(a, 2) ^ rotate(a, 13) ^ rotate(a, 22);
      const t2 = (s0 + ((a & b) ^ (a & c) ^ (b & c))) | 0;
      [a,b,c,d,e,f,g,q] = [(t1 + t2) | 0,a,b,c,(d + t1) | 0,e,f,g];
    }
