...
// It is the one field in the shape that is null rather than empty when unset,
// and it is derived: it appears and disappears with the relation, without the
// issue's own version moving.
...
func TestPatchIgnoresParentButStillChecksIt(t *testing.T) {
	...
	for _, body := range []string{
		`{"title":"renamed","parent":null}`,
		`{"title":"renamed","parent":"` + epic.ID + `"}`,
	} {
		resp, payload := a.do(http.MethodPatch, "/api/issues/"+issue.ID, body)
		require.Equal(t, http.StatusOK, resp.StatusCode, payload)
		assert.Contains(t, payload, `"parent":null`, body)
	}
