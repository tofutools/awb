...
// ActiveProjectExists reports whether a current workspace with this key is
...
func (t *Tx) ActiveProjectExists(key string) (bool, error) {
	visible, args := t.visibleClause("key")
	var one int
	err := t.q.QueryRowContext(t.ctx,
		`SELECT 1 FROM projects WHERE key = ? AND state = 'active' AND `+visible,
		append([]any{key}, args...)...).Scan(&one)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
