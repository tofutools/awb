...
func TestArchivedWorkspaceIsRetainedReadOnlyAndRestorable(t *testing.T) {
	b, ctx := newBackend(t)
	issue := create(t, b, ctx, "retained")
	_, err := b.CreateWorkspace(ctx, backend.WorkspaceCreate{Key: "web"})
	require.NoError(t, err)
	other, err := b.CreateIssue(ctx, backend.IssueCreate{Workspace: "web", Title: "linked"})
	require.NoError(t, err)
	_, err = b.AddRelation(ctx, other.ID, backend.RelationRequest{Type: domain.RelRelated, Other: issue.ID}, "")
	require.NoError(t, err)
	before, err := b.GetWorkspace(ctx, "awb")
	require.NoError(t, err)

	archived, err := b.ArchiveWorkspace(ctx, "awb", backend.ETag(before.UpdatedAt))
	require.NoError(t, err)
	...
	active, err := b.ListWorkspaces(ctx, "", domain.DefaultWorkspaceSort, nil, nil)
	require.NoError(t, err)
	require.Len(t, active.Workspaces, 1)
	assert.Equal(t, "web", active.Workspaces[0].Key)
	history, err := b.ListWorkspacesByState(ctx, "", domain.WorkspacesArchived, domain.DefaultWorkspaceSort, nil, nil)
	require.NoError(t, err)
	require.Len(t, history.Workspaces, 1)
	...
	require.Error(t, err)
	assert.Equal(t, 4, exitOf(err), "manual board/list moves cannot mutate archived workspaces")
	_, err = b.AddAttachment(ctx, issue.ID, backend.AttachmentCreate{Name: "note.txt", Content: strings.NewReader("x")})
	require.Error(t, err)
	assert.Equal(t, 4, exitOf(err))
	_, err = b.CreateIssue(ctx, backend.IssueCreate{Workspace: "awb", Title: "must not exist"})
	require.Error(t, err)
	assert.Equal(t, 4, exitOf(err))
	_, err = b.DeleteWorkspace(ctx, "awb", true, "")
	require.Error(t, err)
	assert.Equal(t, 4, exitOf(err))
	_, err = b.DeleteWorkspace(ctx, "web", true, "")
	...
	_, err = b.ArchiveWorkspace(ctx, "awb", backend.ETag(archived.UpdatedAt))
	require.NoError(t, err)
	_, err = b.ArchiveWorkspace(ctx, "awb", backend.ETag(before.UpdatedAt))
	require.Error(t, err)
	assert.ErrorIs(t, err, awberr.ErrPreconditionFailed)

	restored, err := b.RestoreWorkspace(ctx, "awb", backend.ETag(archived.UpdatedAt))
	require.NoError(t, err)
