...
func TestEnumParameterCompletions(t *testing.T) {
	...
	tests := []struct {
		...
		{"install skills harness", []string{"install-skills", "--harness"}, []string{"all", "claude", "codex", "opencode", "copilot"}},
	}
...
func TestInstallSkillsCommand(t *testing.T) {
	...
	out := h.mustRun("install-skills", "--harness", "claude")
	...
	_, _, code := h.run("install-skills", "--harness", "unknown")
	assert.Equal(t, 2, code)
}
